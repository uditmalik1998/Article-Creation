...
export default function BodyArticleDashboard({ pathType }: BodyArticleDashboardProps = {}) {
  ...
  const exportHeaders = useMemo(
    () => pathType === 'created'
      ? [...SIMPLE_BODY_EXPORT_HEADERS, 'Approved By Name', 'Approved By Email']
      : [...SIMPLE_BODY_EXPORT_HEADERS],
    [pathType],
  );
        ...
        <>
          <div className="grid grid-cols-2 gap-3 p-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {items.map((item, index) => (
              <BodyArticleCard key={item.id} item={item} index={index} onClick={handleCardClick}
