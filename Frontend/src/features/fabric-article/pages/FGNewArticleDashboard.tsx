...
interface FGNewArticleDashboardProps {
  pathType?: 'new' | 'old' | 'rejected' | 'created' | 'failed';
}

export default function FGNewArticleDashboard({ pathType = 'new' }: FGNewArticleDashboardProps) {
  ...
  const fetchItems = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        params.set('pathType', pathType);
        if (pathType !== 'created' && statusFilter !== 'ALL') params.set('status', statusFilter);
    ...
    [statusFilter, divisionFilter, subDivisionFilter, majorCategoryFilter, searchText, dateRangeFilter, pathType],
  );
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      ...
      const fileName =
        pathType === 'new' ? 'FG New Articles' : pathType === 'old' ? 'FG Old Articles'
        : pathType === 'rejected' ? 'FG Rejected Articles' : pathType === 'created' ? 'FG Created Articles' : 'FG Articles';
  ...
  const handleCardClick = useCallback((item: ApproverItem, index: number) => {
    ...
    const basePath =
      d.pathType === 'old' ? '/fabric-article/fg-old'
      : d.pathType === 'rejected' ? '/fabric-article/fg-rejected'
      : d.pathType === 'created' ? '/fabric-article/fg-created'
      : d.pathType === 'failed' ? '/fabric-article/fg-failed'
      : '/fabric-article/fg-new';
  ...
  const pageTitle =
    pathType === 'new' ? 'FG New Articles'
    : pathType === 'old' ? 'FG Old Articles'
    : pathType === 'rejected' ? 'FG Rejected Articles'
    : pathType === 'created' ? 'FG Created Articles'
    : pathType === 'failed' ? 'FG Failed Creations'
    : 'FG Articles';
    ...
    <div className="flex flex-col">
      <div className="sticky top-0 z-30 mb-2 -mx-1 px-1 pt-1">
        <div className="overflow-hidden rounded-xl border border-white/60 bg-white/85 shadow-[var(--shadow-md)] backdrop-blur">
          <div
            ...
            style={{ background: 'linear-gradient(90deg, #3b1f5e 0%, #6b35b0 100%)' }}
          >
            <div className="flex min-w-0 items-center gap-2.5">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-violet-400/90">
            ...
            <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">
              {selectedIds.size > 0 && (
                <>
                  <span className="rounded-md bg-violet-400/90 px-2 py-0.5 text-[11px] font-semibold tabular-nums">
          ...
          <div className="border-t border-border/60 bg-gradient-to-b from-slate-50/40 to-transparent px-3 py-1.5">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {(showSubDivisionFilter || isUnscoped) && (
                <Popover open={subDivOpen} onOpenChange={(o) => { setSubDivOpen(o); if (!o) setSubDivSearch(''); }}>
                  <PopoverTrigger asChild>
                    <button type="button" className="flex h-7 w-[130px] items-center justify-between rounded border border-input bg-background px-2 text-[12px] hover:border-ring focus:outline-none focus:ring-1 focus:ring-ring">
              ...
              <Popover open={majCatOpen} onOpenChange={(o) => { setMajCatOpen(o); if (!o) setMajCatSearch(''); }}>
                <PopoverTrigger asChild>
                  <button type="button" className="flex h-7 w-[170px] items-center justify-between rounded border border-input bg-background px-2 text-[12px] hover:border-ring focus:outline-none focus:ring-1 focus:ring-ring">
      ...
      {loading ? (
        ...
        <div className="flex h-64 flex-col items-center justify-center gap-2 text-muted-foreground">
          ...
          <span className="text-sm">No FG articles found</span>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 p-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {items.map((item, index) => (
              <FabricArticleCard key={item.id} item={item} index={index} onClick={handleCardClick}
